...
class Deck(SlotSet):
    ...
    :param path: The path to the deck configuration `.json` files for the labware,defaults to the 'deck_definition/' in the science_jubilee/decks directory.
    ...
    def safe_z(self):
        ...
        :return: The height at which the pipette can freely move without colliding with labware on the deck.
        :rtype: float
        """
        return self._safe_z
    ...
    @safe_z.setter
    def safe_z(self, val):
        """Function that updates the movement clearance height every time a new labware is loaded onto the deck
